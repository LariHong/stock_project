...
class App(ThemedTk):
    ...
    def create_new_tab(self, tab_name, data: pd.DataFrame):
        ...
        model_button = ttk.Button(top_frame, text="模型和預測", command=lambda:self.model_choose_button(data))
        model_button.grid(row=1, column=4, padx=10, pady=10, sticky="w")
    ...
    def candlestick_chat_graph(self,data,frame):
        ...
        # 设置标题和标签
        ax.set_title('K線圖與折線圖', fontproperties=font)
        ax.set_xlabel('日期')
        ax.set_ylabel('Price')
    ...
    def show_heat_map(self, data):
        ...
        heat_map_frame.pack(fill='both',expand=True)
        ...
        self.choose_function_combobox.pack()
    ...
    def feature_score(self,data,frame,event=None):
        ...
        self.f_regression_feature=col[scoresort[:5]]

    def model_choose_button(self,data):
        top_window = tk.Toplevel(self)
    ...
    def model_combobox(self,frame,combobox_value,data):
        combobox_label = ttk.Label(frame, text="Select option:")
        combobox_label.grid(row=1, column=0, padx=10, pady=10)

        model_combobox = ttk.Combobox(frame)
        model_combobox.grid(row=1, column=1, padx=10, pady=10)
        model_combobox['values'] = combobox_value

        model_combobox.bind("<<ComboboxSelected>>", lambda event: self.model_functon(model_combobox.get(),data,self.f_regression_feature))
    
    def model_functon(self,model,data,feature):
        result_textbox = scrolledtext.ScrolledText(self.result_frame, width=70, height=10)
        result_textbox.grid(row=0, column=0, columnspan=2, padx=10, pady=10)
        ...
        if model=="決策樹(R)":
            ...
            result_textbox.insert(tk.END, f"Decision_tree(R) 均方誤差(mse): {mse}\n")
            result_textbox.insert(tk.END, f"Decision_tree(R) R^2(r2): {r2}\n")
            result_textbox.insert(tk.END, f"Decision_tree(R) y_predict: {y_predict}\n")
